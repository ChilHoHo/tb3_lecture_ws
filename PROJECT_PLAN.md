...
| 成功落点精度（多数 run） | **≈ 0.02–0.05 m**（map 系位姿 vs 目标 (5.3, -2.95)） |
| 完整往返（门→讲台→门） | 回程物理误差 **≈ 0.23 m**（gz 真值；改前偏 ~5 m / 卡死） |
| 定位一致性 | map 与 gz 物理真值一致；纯 gz 轮式里程计漂 ~7.8 m 由 SLAM+闭环纠正 |
| 已修复的偶发 | **SLAM 地图锚点漂移**（map 系落点 y 偏 ~3 m）：已通过**开 IMU 融合 + 加强全局闭环 + 回程走去程逆路线**解决（详见 DEVELOPMENT_LOG §H）；仍在项目中的移动 actor 其近场干扰是后续可优化点 |
