...
## 3.5 预期表现与验收
- 正常一次「门口→讲台」约 **35–50 秒**（机器人在 RT≈0.7~1 的仿真下）。若超过 2 分钟仍在走，可 `stop` 后 `go` 重规划一次。
- 到达判据：状态文字变 **ARRIVED** 且 RViz 里机器人贴合红色讲台圆柱（map 位姿 ≈ (5.3,-2.95)）。
- 已知边界：早期曾有个别 run 因 **SLAM 地图锚点漂移**导致"到了但 map 显示偏差大"；已通过**开 IMU 融合 + 加强全局闭环 + 回程走去程逆路线**修复（见 DEVELOPMENT_LOG §H）。现场若仍遇定位异常，`return` 回门口再 `go` 通常即可。
- **自动验收**（无头，适合演示前自检）：
  ```bash
  ~/tb3_lecture_ws/scripts/run_regression.sh 3   # 自动跑 3 次并汇总 成功/用时/误差
  ```
